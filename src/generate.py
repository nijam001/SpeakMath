# Creating modular SpeakMath package files and a single-file SpeakMath.py, then running tests.
from pathlib import Path
pkg_dir = Path("speakmath")
...
# --- llm_layer.py --- (simple simulation)
llm_layer_code = r'''
...
class Interpreter:
    ...
    def execute_map(self, node: ast.MapNode):
        ...
        out = []
        if op == "OP_MAP" or op == "map" or op == "OP_MAP_ADD":
...
# --- single-file SpeakMath.py --- create by concatenating modules for convenience
single_code = "# SpeakMath single-file version\n\n"
single_code += "# semantic_map\n" + semantic_map_code + "\n\n"
single_code += "# llm_layer\n" + llm_layer_code + "\n\n"
single_code += "# lexer\n" + lexer_code + "\n\n"
single_code += "# ast\n" + ast_code + "\n\n"
single_code += "# parser\n" + parser_code + "\n\n"
single_code += "# interpreter\n" + interpreter_code + "\n\n"
single_code += "# simple demo\n" + main_code + "\n\n"


# Write all files
files = {
    "semantic_map.py": semantic_map_code,
    "llm_layer.py": llm_layer_code,
    "lexer.py": lexer_code,
    "ast.py": ast_code,
    "parser.py": parser_code,
    "interpreter.py": interpreter_code,
    "main.py": main_code,
    "SpeakMath.py": single_code,
}
...
# Run some tests by importing the package via sys.path hack
import sys
sys.path.insert(0, str(pkg_dir.parent))  # so 'speakmath' package can be imported
# create a simple package __init__.py
(pkg_dir / "__init__.py").write_text("from .main import run_command, demo\n")

# Now import and run demo
import importlib
speak = importlib.import_module("speakmath.main")
speak.demo()

print("\nFiles created in:", pkg_dir)
print("Single-file version at:", pkg_dir / "SpeakMath.py")
